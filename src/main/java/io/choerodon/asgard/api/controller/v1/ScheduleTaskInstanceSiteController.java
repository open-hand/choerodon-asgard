...
public class ScheduleTaskInstanceSiteController {
    ...
    public ResponseEntity<PageInfo<ScheduleTaskInstance>> pagingQuery(@RequestParam(value = "status", required = false) String status,
        ...
        return scheduleTaskInstanceService.pageQuery(pageable.getPageNumber(), pageable.getPageSize(), status, taskName, exceptionMessage, params, ResourceLevel.SITE.value(), 0L);
    }
    ...
    public ResponseEntity<PageInfo<ScheduleTaskInstanceLog>> pagingQueryByTaskId(@PathVariable long taskId,
                                                                                 ...
                                                                                 @ApiIgnore
                                                                                 @SortDefault(value = "id", direction = Sort.Direction.DESC) Pageable pageable) {
        return new ResponseEntity<>(scheduleTaskInstanceService.pagingQueryByTaskId(pageable.getPageNumber(), pageable.getPageSize(), taskId, status, serviceInstanceId, params, ResourceLevel.SITE.value(), 0L), HttpStatus.OK);
    }
