...
class SagaServiceSpec extends Specification {
    ...
    def '测试 create方法'() {
        given: '创建测试saga对象'
        def saga = new SagaDTO(null, 'iam', 'test', '{}', 'test')
    ...
    def '测试 query方法'() {
        given: '创建测试数据'
        def saga = new SagaDTO('saga', 'iam', 'desc', '{}', 'test')
        def task1 = new SagaTaskDTO('one', 'saga', 2, true, 'iam')
        def task2 = new SagaTaskDTO('two', 'saga', 6, true, 'iam')
        def task3 = new SagaTaskDTO('three', 'saga', 6, true, 'iam')
    ...
    def '测试 delete方法'() {
        ...
        mockSagaMapper.selectByPrimaryKey(_) >> new SagaDTO('code')
        ...
        mockSagaTaskMapper.select(_) >> [new SagaTaskDTO()]
        mockSagaMapper.selectByPrimaryKey(_) >> new SagaDTO('code')
        sagaService.delete(1L)
