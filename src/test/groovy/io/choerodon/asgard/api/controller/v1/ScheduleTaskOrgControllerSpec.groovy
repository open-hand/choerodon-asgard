...
class ScheduleTaskOrgControllerSpec extends Specification {
    ...
    def "Create"() {
        ...
        def response = restTemplate.postForEntity(BASE_PATH, inVaildScheduleTask, QuartzTaskDTO)
        then: '状态码验证通过；验证方法参数不生效；验证异常正确抛出'
        response.statusCode.is2xxSuccessful()
        0 * mockScheduleTaskService.create(_, _, _)

        when: 'POST请求【创建定时任务】-结束时间为过去时间'
        response = restTemplate.postForEntity(BASE_PATH, scheduleTask, Object)
        ...
        response = restTemplate.postForEntity(BASE_PATH, scheduleTask, QuartzTaskDTO)
    ...
    def "GetTaskDetail"() {
        ...
        def id = 1L
        when: 'GET请求【查看任务详情】'
        def response = restTemplate.getForEntity(BASE_PATH + "/{id}", ScheduleTaskDetail, id)
        then: '状态码验证成功；参数验证合法'
        response.statusCode.is2xxSuccessful()
